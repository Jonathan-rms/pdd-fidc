...
<style>
    ...
    * { font-family: 'Montserrat', sans-serif !important; }
    ...
    div[data-testid="stFileUploader"] svg { fill: #000000 !important; color: #000000 !important; }

    .custom-metric-box { background-color: transparent; padding: 0px; }
    ...
    div[data-testid="stDownloadButton"] > button:hover { color: #ffffff !important; }
    [data-testid="stInfo"] {
    ...
    .styled-table th { background-color: #0030B9; color: white; padding: 12px 15px; text-align: left; }
    .styled-table td { padding: 10px 15px; border-bottom: 1px solid #f0f0f0; color: #333; }
    .styled-table tr.total-row td {
        font-weight: 700; background-color: #f4f8ff;
        border-top: 2px solid #0030B9; color: #0030B9;
    }
...
# --- 2. REGRAS (Política 2026.1 — 16 ratings) ---
# Ordem crescente de risco: AA → A → BB → B → CC → C → DD → D → EE → E → FF → F → GG → G → HH → H
REGRAS = pd.DataFrame({
    'Rating': ['AA', 'A', 'BB', 'B', 'CC', 'C', 'DD', 'D', 'EE', 'E', 'FF', 'F', 'GG', 'G', 'HH', 'H'],
    '% Nota': [0.0,  0.005, 0.007, 0.01, 0.02, 0.03, 0.05, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.85, 1.0],
    '% Venc': [1.0,  0.995, 0.993, 0.99, 0.98, 0.97, 0.95, 0.90, 0.80, 0.70, 0.60, 0.50, 0.40, 0.30, 0.15, 0.0]
})

# Ordem para exibição/sort
RATING_ORDER = {k: v for v, k in enumerate(REGRAS['Rating'])}
...
def calcular_dataframe(df, idx):
    """
    Política 2026.1:
    - PDD Nota: pro rata de aquisição até vencimento
    - PDD Vencido: escalonamento linear de dia 61 a dia 90 (janela de 30 dias)
      ≤60 dias → 0% | 61-89 dias → (atraso-60)/30 | ≥90 dias → 100%
    """
    df_calc = df.copy()
    ...
    # ── MUDANÇA 2026.1: janela 61-90 dias (era 21-60) ──────────────────────────
    pr_v = np.select(
        [atr <= 60, atr >= 90],
        [0.0,        1.0],
        default=((atr - 60) / 30)
...
def gerar_excel_final(df_original, calc_data):
    ...
    for batch_start in range(0, total_rows, batch_size):
        ...
        for i in range(batch_start, batch_end):
            ...
            write(row, col_dias_aq_venc,
                  f'={L["venc"]}{r}-{L["aq"]}{r}', f_num)
            write(row, col_dias_atraso,
                  f'={L["pos"]}{r}-{L["venc"]}{r}', f_num)
            write(row, col_pdd_nota,
                  f'=VLOOKUP({L["rat"]}{r},Regras_Sistema!$A:$C,2,0)', f_pct)
            write(row, col_pdd_nota_prorata,
                  f'=IF({L["rat"]}{r}="H",1,IF({CL_dias_aq_venc}{r}=0,0,'
                  f'MIN(1,MAX(0,({L["pos"]}{r}-{L["aq"]}{r})/{CL_dias_aq_venc}{r}))))', f_pct)
            write(row, col_pdd_nota_final,
                  f'=IF({CL_pdd_nota_prorata}{r}=0,{CL_pdd_nota}{r},'
                  f'{CL_pdd_nota}{r}*{CL_pdd_nota_prorata}{r})', f_pct)
            write(row, col_pdd_venc,
                  f'=VLOOKUP({L["rat"]}{r},Regras_Sistema!$A:$C,3,0)', f_pct)

            # ── MUDANÇA 2026.1: janela 61-90 dias (era 21-60) ────────────────
            write(row, col_pdd_venc_prorata,
                  f'=IF({CL_dias_atraso}{r}<=60,0,'
                  f'IF({CL_dias_atraso}{r}>=90,1,'
                  f'({CL_dias_atraso}{r}-60)/30))', f_pct)
            # ─────────────────────────────────────────────────────────────────

            write(row, col_pdd_venc_final,
                  f'={CL_pdd_venc}{r}*{CL_pdd_venc_prorata}{r}', f_pct)
            write(row, col_pdd_nota_calc,
                  f'={L["val"]}{r}*{CL_pdd_nota_final}{r}', f_money)
            write(row, col_dif_nota,
                  f'=ABS({CL_pdd_nota_calc}{r}-{orig_n_base}{r})', f_money)
            write(row, col_pdd_venc_calc,
                  f'={L["val"]}{r}*{CL_pdd_venc_final}{r}', f_money)
            write(row, col_dif_venc,
                  f'=ABS({CL_pdd_venc_calc}{r}-{orig_v_base}{r})', f_money)
    ...
    classes = sorted(
        [str(x) for x in df_clean.iloc[:, idx['rat']].unique() if str(x) != 'nan'],
        key=lambda x: RATING_ORDER.get(x, 99)
    )
    r_idx = 1
    for cls in classes:
        row  = str(r_idx + 1)
...
<div style='text-align: center; margin-bottom: 20px;'>
    ...
    <p style='color:grey; font-size:14px'>CÁLCULO DE PROVISÃO (PDD) — Política 2026.1</p>
</div>
""", unsafe_allow_html=True)
...
if uploaded_file:
    ...
    if (st.session_state.current_file_name != uploaded_file.name
        ...
        if err:
            ...
            def get_col(keys):
                return next((df_raw.columns.get_loc(c) for c in df_raw.columns
                             if any(k in c.lower().replace('_','') for k in keys)), None)

            idx = {
                'aq':  get_col(['aquisicao']),
                'venc':get_col(['vencimento']),
                'pos': get_col(['posicao']),
                'rat': get_col(['notapdd','classificacao']),
...
if st.session_state.processed_data:
    ...
    agg_dict = {df.columns[idx['val']]: 'sum', 'CALC_N': 'sum', 'CALC_V': 'sum'}
    if idx['orn']: agg_dict[df.columns[idx['orn']]] = 'sum'
    if idx['orv']: agg_dict[df.columns[idx['orv']]] = 'sum'

    df_grp = df.groupby(rat_name).agg(agg_dict)
    df_grp['sort'] = df_grp.index.map(RATING_ORDER).fillna(99)
    ...
    # Montar colunas de exibição dinamicamente
    show_cols = {"Valor Presente": df.columns[idx['val']]}
    if idx['orn']: show_cols["PDD Nota (Orig)"]  = df.columns[idx['orn']]
    show_cols["PDD Nota (Calc)"]  = 'CALC_N'
    if idx['orv']: show_cols["PDD Venc (Orig)"]  = df.columns[idx['orv']]
    show_cols["PDD Venc (Calc)"]  = 'CALC_V'

    df_show = pd.DataFrame({label: df_grp[col].map(fmt) for label, col in show_cols.items()})
    st.markdown(make_html_table(df_show, idx_col_name="Rating"), unsafe_allow_html=True)

    st.markdown('<div class="spacer-lg"></div>', unsafe_allow_html=True)

    with st.info("📚 **Regras e Lógica de Cálculo** — Política 2026.1"):
        col_regras, col_logica = st.columns(2)

        with col_regras:
            st.markdown("**Tabela de Parâmetros (16 ratings)**")
        ...
        with col_logica:
            ...
            <div class="logic-box">
                ...
                <br>
                <strong style="color:#0030B9">2. PDD Vencido (Atraso) — 2026.1</strong>
                <ul style="font-size:0.9em; padding-left:20px; color:#444; margin-top:5px; line-height:1.6;">
                    <li><b>≤ 60 dias:</b> 0%</li>
                    <li><b>61 a 89 dias:</b> Escalonamento linear<br>
                        <span style="font-size:0.85em; color:#666; background:#f4f4f4; padding:2px 6px; border-radius:4px;">(Dias Atraso - 60) ÷ 30</span>
                    </li>
                    <li><b>≥ 90 dias:</b> 100% de provisionamento</li>
                </ul>
                <p style="font-size:0.8em; color:#888; margin-top:4px;">
                    ⚠️ Janela alterada em relação à versão anterior (era 21-60 dias).
                </p>
            </div>
            """, unsafe_allow_html=True)
